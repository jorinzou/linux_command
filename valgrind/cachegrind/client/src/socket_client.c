...
int connect_server(int port ,const char *ipaddr,int sockfd)
{
    ...
    if (ret < 0){
        perror("connect");
        DEBUG_INFO("connect server failed11");
        return -1;
    }
     /*设置成非阻塞*/
    unsigned long ul = 1;
    //ioctl(g_sockfd, FIONBIO, &ul); 
    DEBUG_INFO("connect server sucessful22");
    return 0;
}
...
void do_topic_and_msg(const uint8* topic,const uint8* msg)
{
    ...
    char *str_json = cJSON_Print(msg_json);
    //free(str_json);
    //cJSON_Delete(msg_json);
    //DEBUG_INFO("%s",str_json);
}
...
void *client_do_pthread(void *arg)
{
    ...
    int ret;//发送三次握手报文
    ret = connect_server(SERVER_PORT,ip_str,g_sockfd);
    if(ret < 0){
